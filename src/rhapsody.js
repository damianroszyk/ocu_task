...
(function(exports, $, JSON) {
	...
	var Rhapsody = {
		...
		init: function(options) {
			...
			Rhapsody.member.accessToken = ACCESS_TOKEN_KEY;
			...
			if (id && typeof id === 'string') {
				...
				console.log("id", id);
				var iframeElement = document.getElementById('player-frame');
				if (!iframeElement) {
					var newIframe = document.createElement("iframe");
					newIframe.setAttribute("id", id);
					newIframe.setAttribute("name", id);
					newIframe.setAttribute("src", "http://api.rhapsody.com/v1.1/player/index.html?apikey=" + options.consumerKey);
					newIframe.setAttribute("frameborder", "no");
					newIframe.setAttribute("style", "display:none;");
					document.getElementsByTagName('BODY')[0]
						.appendChild(newIframe);
					setTimeout(function() {
						iframeElement = document.getElementById('player-frame');
						that.player.win = document.getElementById('player-frame');
					});
				} else if (document.getElementById('player-frame') instanceof HTMLIFrameElement) {
					that.player.win = document.getElementById('player-frame');
				} else {
		...
		api: {
			...
			headers: function(secure) {
				...
				// if (secure && Rhapsody.member.accessToken) {
					h['Authorization'] = 'Bearer ' + 'ZWZmYWNjYzQtYTljNy00N2IwLWJkMjUtNTgxZDZlOGMzMzNj';
				// }
		...
		player: {
			...
			auth: function() {
				if (Rhapsody.api.consumerKey && Rhapsody.member.accessToken) {
					Rhapsody.windows(this.win).post('auth', {
						consumerKey: Rhapsody.api.consumerKey,
						accessToken: Rhapsody.member.accessToken
					});
				}
			},
